...
static struct hal_s ghalStruct = {
    0
};
...
static void read_from_mpl(void)
{
    ...
    if (inv_get_sensor_type_quat(data, &accuracy, (inv_time_t*)&timestamp)) {
        ...
        if (ghalStruct.report & PRINT_QUAT) {
            eMPL_send_data(PACKET_DATA_QUAT, data);
        }
    }

    if (ghalStruct.report & PRINT_ACCEL) {
    ...
    if (ghalStruct.report & PRINT_GYRO) {
    ...
    if (ghalStruct.report & PRINT_COMPASS) {
...
#endif
    if (ghalStruct.report & PRINT_EULER) {
    ...
    if (ghalStruct.report & PRINT_ROT_MAT) {
    ...
    if (ghalStruct.report & PRINT_HEADING) {
    ...
    if (ghalStruct.report & PRINT_LINEAR_ACCEL) {
        if (inv_get_sensor_type_linear_acceleration(float_data, &accuracy, (inv_time_t*)&timestamp)) {
            MPL_LOGI("Linear Accel: %7.5f %7.5f %7.5f\r\n",
                     float_data[0], float_data[1], float_data[2]);
        }
    }
    if (ghalStruct.report & PRINT_GRAVITY_VECTOR) {
    ...
    if (ghalStruct.report & PRINT_PEDO) {
        unsigned long timestamp;
        timestamp = os::Task::getTickCount();
        if (timestamp > ghalStruct.next_pedo_ms) {
            ghalStruct.next_pedo_ms = timestamp + PEDO_READ_MS;
...
static void setup_gyro(void)
{
    ...
    if (ghalStruct.sensors & ACCEL_ON) {
        mask |= INV_XYZ_ACCEL;
    }
    if (ghalStruct.sensors & GYRO_ON) {
        mask |= INV_XYZ_GYRO;
        lp_accel_was_on |= ghalStruct.lp_accel_mode;
    }
#ifdef COMPASS_ENABLED
    if (ghalStruct.sensors & COMPASS_ON) {
...
static void handle_input(void)
{
    ...
    case 'a':
        ghalStruct.report ^= PRINT_ACCEL;
        break;

    case 'g':
        ghalStruct.report ^= PRINT_GYRO;
        break;

#ifdef COMPASS_ENABLED
    case 'c':
        ghalStruct.report ^= PRINT_COMPASS;
        break;

#endif
    case 'e':
        ghalStruct.report ^= PRINT_EULER;
        break;

    case 'r':
        ghalStruct.report ^= PRINT_ROT_MAT;
        break;

    case 'q':
        ghalStruct.report ^= PRINT_QUAT;
        break;

    case 'h':
        ghalStruct.report ^= PRINT_HEADING;
        break;

    case 'i':
        ghalStruct.report ^= PRINT_LINEAR_ACCEL;
        break;

    case 'o':
        ghalStruct.report ^= PRINT_GRAVITY_VECTOR;
        break;
    ...
    case 'p':
         ...
         * misses the rising/falling edge, and the ghalStruct.new_gyro flag is never
        ...
        ghalStruct.sensors &= ~(GYRO_ON | COMPASS_ON);
        ghalStruct.sensors |= ACCEL_ON;
        ghalStruct.lp_accel_mode = 1;
        inv_gyro_was_turned_off();
        inv_compass_was_turned_off();
        break;
    ...
    case 'v':
         ...
         */
        ghalStruct.dmp_features ^= DMP_FEATURE_6X_LP_QUAT;
        dmp_enable_feature(ghalStruct.dmp_features);
        if (!(ghalStruct.dmp_features & DMP_FEATURE_6X_LP_QUAT)) {
            inv_quaternion_sensor_was_turned_off();
...
{
    ghalStruct.new_gyro = 1;
}

extern "C" void MpuInterruptHandler(void)
{
    gyro_data_ready_cb();
}
/*******************************************************************************/

const os::TaskEndless app::emplTest("eMPL_Demo", 1024, 4, [](const bool&){
                                    constexpr auto& exti = hal::Factory<hal::Exti>::get<hal::Exti::IMU_INT>();
...
#ifdef COMPASS_ENABLED
                                    ghalStruct.sensors = ACCEL_ON | GYRO_ON | COMPASS_ON;
#else
                                    ghalStruct.sensors = ACCEL_ON | GYRO_ON;
#endif
                                    ghalStruct.dmp_on = 0;
                                    ghalStruct.report = 0;
                                    ghalStruct.rx.cmd = 0;
                                    ghalStruct.next_pedo_ms = 0;
                                    ghalStruct.next_compass_ms = 0;
                                    ghalStruct.next_temp_ms = 0;
                                     ...
                                     */
                                    ghalStruct.dmp_features = DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
                                    ...
                                    while (1) {
                                         ...
                                         */
                                        if ((timestamp > ghalStruct.next_compass_ms) &&
                                            !ghalStruct.lp_accel_mode &&
                                            ghalStruct.new_gyro && (ghalStruct.sensors & COMPASS_ON))
                                        {
                                            ghalStruct.next_compass_ms = timestamp + COMPASS_READ_MS;
                                            new_compass = 1;
                                        }
#endif
                                         ...
                                         */
                                        if (timestamp > ghalStruct.next_temp_ms) {
                                            ghalStruct.next_temp_ms = timestamp + TEMP_READ_MS;
                                            new_temp = 1;
                                        }

                                        if (ghalStruct.motion_int_mode) {
                                            ...
                                            while (!ghalStruct.new_gyro) {}
                                            /* Restore the previous sensor configuration. */
                                            mpu_lp_motion_interrupt(0, 0, 0);
                                            ghalStruct.motion_int_mode = 0;
                                        }

                                        if (!ghalStruct.sensors || !ghalStruct.new_gyro) {
                                            continue;
                                        }

                                        if (ghalStruct.new_gyro && ghalStruct.lp_accel_mode) {
                                            short accel_short[3];
                                            long accel[3];
                                            ...
                                            new_data = 1;
                                            ghalStruct.new_gyro = 0;
                                        } else if (ghalStruct.new_gyro && ghalStruct.dmp_on) {
