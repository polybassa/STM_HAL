...
void FilterOnePole::setFilter(FILTER_TYPE ft, float fc, float initialValue)
{
    ...
    LastUS = os::Task::getTickCount();
}

float FilterOnePole::input(float inVal)
{
    long time = os::Task::getTickCount();
...
void FilterOnePole::test()
{
    ...
    float nextupdateTime = os::Task::getTickCount() * 1e-3;
    ...
    while (true) {
        float now = os::Task::getTickCount() * 1e-3;
