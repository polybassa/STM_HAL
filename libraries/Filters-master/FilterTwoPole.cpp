...
float FilterTwoPole::input(float drive)
{
    ...
    long now = os::Task::getTickCount();                      // get current time
    float dt = 1e-3 * float(now - LastTimeUS);  // find dt
    ...
    dt = constrain<float>(dt, 0, 1.0 / W0);

    float A = std::pow(W0, 2) * drive - W0 / Q * Vprev - std::pow(W0, 2) * X; // *** compute acceleration
...
float FilterTwoPole::getMaxAmp()
{
    ...
    float E = 0.5* W0* std::pow(X, 2) + 0.5 * std::pow(Vprev, 2) / W0;
...
void FilterTwoPole::test()
{
    ...
    float nextupdateTime = 1e-3 * float(os::Task::getTickCount());

    float inputValue = 0;
    FilterTwoPole osc(0.2, 4, 0);

    while (true) {
        float now = 1e-3 * float(os::Task::getTickCount());
